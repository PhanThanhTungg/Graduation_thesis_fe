...
const SummaryCard = ({ title, value, increase, icon, color }: SummaryCardProps) => {
    ...
    <div className={`bg-card rounded-xl p-6 border border-border shadow-sm hover:shadow-md transition-shadow`}>
      <div className="flex items-start justify-between mb-4">
        ...
        <div className={`flex items-center gap-1 px-2 py-1 rounded-full text-sm font-medium ${
          isPositive ? 'bg-green-foreground text-green' : 'bg-red-50 text-red-500'
        }`}>
          {isPositive ? <TrendingUp className="w-4 h-4" /> : <TrendingDown className="w-4 h-4" />}
          <span>{Math.abs(increase)}%</span>
        </div>
      </div>
      <h3 className="text-muted-foreground text-sm font-medium mb-1">{title}</h3>
      <p className="text-3xl font-bold font-heading text-foreground">{value}</p>
      <p className="text-xs text-muted-foreground mt-2">
        {isPositive ? '+' : '-'}{Math.abs(increase)}% from yesterday
      </p>
    </div>
  );
};
