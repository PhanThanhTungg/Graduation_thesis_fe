...
export function EditVoucherForm({ voucher, coursePrice, onSuccess }: EditVoucherFormProps) {
  const [isLoading, setIsLoading] = useState(false);

  const form = useForm<UpdateVoucherType>({
  ...
  const onSubmit = async (data: UpdateVoucherType) => {
    setIsLoading(true);
    try {
      ...
      const updateData: UpdateVoucherType = {};
      
      if (data.code !== voucher.code) updateData.code = data.code;
      if (data.discountType !== voucher.discountType) updateData.discountType = data.discountType;
      if (data.discountValue !== voucher.discountValue) updateData.discountValue = data.discountValue;
      if (data.status !== voucher.status) updateData.status = data.status;
      if (data.usageLimit !== voucher.usageLimit) updateData.usageLimit = data.usageLimit;
      
      // Convert dates to ISO string with time
      if (data.startDate) {
        const startDateTime = new Date(data.startDate);
        startDateTime.setHours(0, 0, 0, 0);
        if (startDateTime.toISOString() !== voucher.startDate) {
      ...
      if (data.endDate) {
        ...
        if (endDateTime.toISOString() !== voucher.endDate) {
          updateData.endDate = endDateTime.toISOString();
        }
      }

      await updateVoucher(voucher.id, updateData);
  ...
  const validateDiscountValue = (value: number | undefined) => {
    if (!value || value <= 0) {
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormDescription>
                Unique code that users will enter to apply the discount
              </FormDescription>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormDescription>
                Control whether the voucher is active or not
              </FormDescription>
              <FormMessage />
            </FormItem>
          )}
